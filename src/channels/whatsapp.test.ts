...
vi.mock('../image-description.js', () => ({
  isImageMessage: vi.fn((msg: any) => !!msg.message?.imageMessage),
  describeImageMessage: vi.fn().mockResolvedValue('grilled chicken ~6oz, white rice ~1 cup, broccoli ~0.5 cup'),
}));
...
import { describeImageMessage } from '../image-description.js';
...
describe('WhatsAppChannel', () => {
  ...
  describe('message handling', () => {
    ...
    it('describes imageMessage and includes caption', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
      ...
      expect(describeImageMessage).toHaveBeenCalled();
      expect(opts.onMessage).toHaveBeenCalledWith(
        'registered@g.us',
        expect.objectContaining({
          content: '[Photo: grilled chicken ~6oz, white rice ~1 cup, broccoli ~0.5 cup]\n(User caption: Check this photo)',
        }),
      );
    });
    ...
    it('describes image messages', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
    ...
    it('includes caption with image description', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
      ...
      await triggerMessages([
        {
          ...
          message: {
            imageMessage: { mimetype: 'image/jpeg', caption: 'My lunch today' },
          },
          pushName: 'Alice',
          messageTimestamp: Math.floor(Date.now() / 1000),
        },
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'registered@g.us',
        expect.objectContaining({
          content: '[Photo: grilled chicken ~6oz, white rice ~1 cup, broccoli ~0.5 cup]\n(User caption: My lunch today)',
        }),
      );
    });

    it('falls back when image description returns null', async () => {
      vi.mocked(describeImageMessage).mockResolvedValueOnce(null);
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'registered@g.us',
        expect.objectContaining({
          content: '[Photo - description unavailable]',
        }),
      );
    });

    it('falls back when image description throws', async () => {
      vi.mocked(describeImageMessage).mockRejectedValueOnce(new Error('API error'));
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'registered@g.us',
        expect.objectContaining({
          content: '[Photo - description failed]',
        }),
      );
    });
