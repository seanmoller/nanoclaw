...
vi.mock('../image-description.js', () => ({
  ...
  describeImageMessage: vi
    .fn()
    .mockResolvedValue(
      'grilled chicken ~6oz, white rice ~1 cup, broccoli ~0.5 cup',
    ),
}));
  ...
  describe('message handling', () => {
    ...
    it('describes imageMessage and includes caption', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content:
            '[Photo: grilled chicken ~6oz, white rice ~1 cup, broccoli ~0.5 cup]\n(User caption: Check this photo)',
        }),
      );
    });
    ...
    it('falls back when image description throws', async () => {
      vi.mocked(describeImageMessage).mockRejectedValueOnce(
        new Error('API error'),
      );
